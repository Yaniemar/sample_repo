...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def from_voice(self, request):
        ...
        parsed = self.parse_voice_command(transcript)
    ...
    def _parse_voice_command(self, transcript):
        ...
        for pattern in time_pattern:
            ...
            if match:
                groups = match.groups()
                hour = int(groups[0])
                minute = int(groups[1]) if len(groups) == 3 and groups[1].isdigit() else 0
                ampm = groups[-1].lower()
        ...
        # Remove filter words
        title = re.sub(r'\b(add|create|schedule|set|remind|me|to|a|an|the|at|on)\b', '', text)
