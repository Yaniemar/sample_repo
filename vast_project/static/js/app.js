...
async function handleAuthSubmit(event) {
    ...
    try {
        const response = await fetch(activeAuthMode === 'signup' ? ENDPOINTS.register : ENDPOINTS.login, {
            method: 'POST',
        ...
        if (!response.ok) {
            ...
            showSystemToast(formatApiError(errorBody));
            return;
        }
...
async function checkBackendHandshake() {
    try {
        ...
        const response = await fetch(ENDPOINTS.summary, { signal: controller.signal });
...
function filterAndRenderDashboard(allTodayTasks, allUpcomingTasks) {
    let filteredToday = [...allTodayTasks];
    let filteredUpcoming = [...allUpcomingTasks];
    ...
    if (currentFilter === 'overdue') {
        // For Overdue, we fetch from our local pool / all tasks
        const sourceList = isOnline ? [...allTodayTasks, ...allUpcomingTasks] : localTasks;
        filteredToday = sourceList.filter(t => t.date < todayStr && t.status !== 'completed');
        filteredUpcoming = sourceList.filter(t => t.date < todayStr && t.status === 'completed');
        
        todayLabel = "Overdue Tasks (Incomplete)";
        upcomingLabel = "Overdue Tasks (Completed)";
    } else if (currentFilter !== 'all') {
        if (currentFilter === 'pending') {
            filteredToday = filteredToday.filter(t => t.status !== 'completed');
            filteredUpcoming = filteredUpcoming.filter(t => t.status !== 'completed');
        } else if (currentFilter === 'completed') {
            filteredToday = filteredToday.filter(t => t.status === 'completed');
            filteredUpcoming = filteredUpcoming.filter(t => t.status === 'completed');
        } else if (currentFilter === 'high') {
            filteredToday = filteredToday.filter(t => t.priority === 'high');
            filteredUpcoming = filteredUpcoming.filter(t => t.priority === 'high');
        }
    }
...
async function processCapturedSpeech(phrase) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorBody = await response.json().catch(() => ({}));
            showSystemToast(`Voice task failed: ${formatApiError(errorBody)}`);
            return;
        }
